...
#
# pipelines:php8.2-alpine docker image
#
# usage: ./php8.2-alpine.sh [<docker-cmd>]
#
...
docker_cmd="${1-docker}"
from="docker.io/php:8.2-alpine"
tag="docker.io/ktomk/pipelines:php8.2-alpine"
...
RUN set -ex ; \
  ...
  readelf -d /usr/local/lib/php/extensions/no-debug-non-zts-20220829/yaml.so | grep 'NEEDED' ; \
...
COPY --from=1 \
  ...
  /usr/local/lib/php/extensions/no-debug-non-zts-20220829/yaml.so \
