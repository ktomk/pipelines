...
#
# pipelines:php8.3-alpine docker image
#
# usage: ./php8.3-alpine.sh [<docker-cmd>]
#
...
RUN set -ex ; \
  ...
  readelf -d /usr/local/lib/php/extensions/no-debug-non-zts-20230831/yaml.so | grep 'NEEDED' ; \
...
COPY --from=1 \
  ...
  /usr/local/lib/php/extensions/no-debug-non-zts-20230831/yaml.so \
