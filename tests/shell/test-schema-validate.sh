...
case ${1-0} in
  ...
  2 ) echo "# 2: validate valid files"
      ...
      build/pipelines.phar --validate --file tests/data/yml/clone.yml
      build/pipelines.phar --validate --file tests/data/yml/condition.yml
      exit
      ;;
  3 ) echo "# 3: validate on syntax error file"
      cd "${PROJECT_DIR}"
      { 2>&1 build/pipelines.phar --validate --file tests/data/yml/error.yml || true; } | grep 'YAML error:'
      exit
      ;;
